...
1.  Starts of with creating a Launch Configuration, which is essentially a template for your instance
2.  Create Auto Scaling Group -> Update Group size to # of instances -> click Subnet and it will autopopulate a list to select from
3.  Advanced Details -> Load Balncing select WebDMZ -> Health Check settings
4.  Configure scaling policies: this actually defines the way your group scales up and down
5.  Change possible limits, set alerts to increase/decrease based on how various attributes (CPU Utilization, etc)
...
- placed on distinc underlying hardware
- recommended for applications that have a small number of critical instances that should be kept separate from each other
- can spread multiple AZ
- "Placement Group" may only refer to Clustered Placement Group in older exams

### EFS Concepts & Lab

File storage service for EC2 instances. EFS provides an interface that allows you to create and configure file systems from a _central repository_. Storage capacity is elastic as you add/remove files.

- Supports NFSv4 protocol
- You only pay for storage you use (no pre-provisioning)
...
1.  Dashboard -> EFS -> check region
2.  Select VPC -> select which AZs in region have mount points -> IP options
3.  Add Tags
4.  Create File System -> Wait for this to process
5.  Click dropdown -> EC2 mount instructions
    - If running Amazon Linux AMI, these dependencies are already installed and you just have to do the `sudo mount` command
